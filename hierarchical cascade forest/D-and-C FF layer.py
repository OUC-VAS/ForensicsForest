...
def _build_estimator(
    ...
    oob = estimator.oob_decision_function_
    ...
    if partial_mode:
        ...
        return X_aug_train, buffer_path, oob
    else:
        return X_aug_train, estimator, oob
...
class ClassificationCascadeLayer(BaseCascadeLayer, ClassifierMixin):
    ...
    def fit_transform(self, X, y, sample_weight=None):
       ...
       # A random forest and an extremely random forest will be fitted
        for estimator_idx in range(self.n_estimators // 2):
            X_aug_, _estimator, oob = _build_estimator(
                X,
                y,
                self.layer_idx,
                estimator_idx,
        ...
        # print(self.oob_decision_function_.shape)
  ...
  # after changing  ##############################################################################################################################################################
    def fit_transform(self, X, y, X_2, y_2, sample_weight=None):
        self._validate_params()
        X_aug = []
        X_aug1 = []
        X_aug2 = []
        ...
        for estimator_idx in range(16):
            ...
            X_aug1.append(X_aug_)
            y_pred1 = np.argmax(oob, axis=1)
            performance_1 = accuracy_score(
                y, y_pred1, sample_weight=sample_weight
            )
            performance1.append(performance_1)
            n_samples, _ = X_2.shape
            pred = np.zeros((n_samples, self.n_outputs * self.n_estimators))
            left, right = self.n_outputs * estimator_idx, self.n_outputs * (estimator_idx + 1)
            y_pred2 = np.argmax(_estimator.predict(X_2), axis=1)
            performance_2 = accuracy_score(
                y_2, y_pred2, sample_weight=sample_weight
            )
            performance2.append(performance_2)
            forest.append(_estimator)
            # key = "{}-{}-{}".format(self.layer_idx, estimator_idx, "rf")
            # self.estimators_.update({key: _estimator})

        performance = [(i + j)/2 for i, j in zip(performance1, performance2)]
        performance = np.array(performance)
        index = performance.argsort()[-2:][::-1]
        index = list(index)
        k = 0
        for i, n in enumerate(forest):

            if i in index:
                key = "{}-{}-{}".format(self.layer_idx, k, "rf")
                self.estimators_.update({key: n})
                k = k + 1

        for i, n in enumerate(list(X_aug1)):
            if i in index:
                X_aug.append(n)
        ...
        for i, n in enumerate(forest):
            if i in index:
                key = "{}-{}-{}".format(self.layer_idx, k, "erf")
                self.estimators_.update({key: n})
                k = k + 1
        for i, n in enumerate(list(X_aug2)):
            if i in index:
                X_aug.append(n)
        # Set the OOB estimations and validation accuracy
        self.oob_decision_function_ = oob_decision_function / self.n_estimators
