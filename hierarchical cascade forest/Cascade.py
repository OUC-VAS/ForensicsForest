...
model1.add_module('fc1', nn.Linear(8, 128))
model1.add_module('fc2', nn.Linear(128, 8))

model1.add_module('fc3', nn.Linear(8, 2))
optimizer = optim.Adam(model1.parameters(), lr=learning_rate)
loss_func = nn.CrossEntropyLoss()

model2 = nn.Sequential()
model2.add_module('fc1', nn.Linear(16, 8))
model2.add_module('fc2', nn.Linear(8, 2))
optimizer = optim.Adam(model2.parameters(), lr=learning_rate)
loss_func = nn.CrossEntropyLoss()

model3 = nn.Sequential()
model3.add_module('fc1', nn.Linear(24, 8))
model3.add_module('fc2', nn.Linear(8, 2))
optimizer = optim.Adam(model3.parameters(), lr=learning_rate)
loss_func = nn.CrossEntropyLoss()

model4 = nn.Sequential()
model4.add_module('fc1', nn.Linear(32, 8))
model4.add_module('fc2', nn.Linear(8, 2))
optimizer = optim.Adam(model4.parameters(), lr=learning_rate)
loss_func = nn.CrossEntropyLoss()

model = nn.Sequential()
model.add_module('fc1', nn.Linear(8, 128))
model.add_module('fc2', nn.Linear(128,8))
model.add_module('fc3', nn.Linear(8,2))
...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit_1Region(self, X, y, sample_weight=None):
        ...
        X_ = self.buffer_.cache_data(0, X_, is_training_data=True)
        # global X_test_
        # X_test_ = self._bin_data(binner_, X_test, is_training_data=False)
        # X_test_ = self.buffer_.cache_data(0, X_test_, is_training_data=False)
        ...
        tic = time.time()
        X_aug_train_ = layer_.fit_transform(
            X_train_, y_train, sample_weight=sample_weight
        )
        # X_aug_train_ = layer_.fit_transform(
        #     X_train_, y_train, X_2, y_2, sample_weight=sample_weight
        # )
        ...
        # print(X_aug2.shape)
        # print(model1)
        # my_tensor = torch.from_numpy(X_aug2)
        # train_features = my_tensor.to(torch.float32)
        #
        # my_tensor = torch.from_numpy(y)
        # train_labels = my_tensor.to(torch.float32)
        #
        ...
        #         loss.backward()
        #         optimizer.step()
        #         output1 = model[0](data)
        #         out = model[1](output1)
        # # out = model1[0](data)
        # X_aug_train_ = out
        # X_aug_train_ = X_aug_train_.detach().numpy()

        # print(model)
        # my_tensor = torch.from_numpy(X_aug)
        # train_features = my_tensor.to(torch.float32)
        #
        # my_tensor = torch.from_numpy(y)
        # train_labels = my_tensor.to(torch.float32)
        ...
        while self.n_layers_ < self.max_layers:
            ...
            tic = time.time()
            X_aug_train_ = layer_.fit_transform(
                X_middle_train_, y_train, sample_weight=sample_weight
            )
            # X_aug_train_ = layer_.fit_transform(
            #     X_middle_train_, y_train, X_2, y_2, sample_weight=sample_weight
            # )
        ...
        if self.use_predictor:
            ...
            if self.verbose > 0:
                ...
                print(msg.format(_utils.ctime(), self.predictor))
            tic = time.time()
            self.predictor_.fit(X_middle_train_, y_train, sample_weight)
            toc = time.time()
            if self.verbose > 0:
                msg = "{} Finish building the predictor | Elapsed = {:.3f} s"
                print(msg.format(_utils.ctime(), toc - tic))
    ...
    def fit_2Region(self, X_train1, X_train2, y, sample_weight=None):
        ...
        self._check_input(X_train1, y)
        self._validate_params()
        self._check_input(X_train2, y)
        self._validate_params()
        n_counter = 0  # a counter controlling the early stopping
        binner_ = Binner(
            n_bins=self.n_bins,
        ...
        # Bin the training data
        X_train_1 = self._bin_data(binner_, X_train1, is_training_data=True)
        X_train_1 = self.buffer_.cache_data(0, X_train_1, is_training_data=True)
        X_train_2 = self._bin_data(binner_, X_train2, is_training_data=True)
        X_train_2 = self.buffer_.cache_data(0, X_train_2, is_training_data=True)
        ...
        tic = time.time()
        X_aug_train_ = layer_.fit_transform(
            X_train_1, y, sample_weight=sample_weight
        )
        ...
        self.n_layers_ += 1
        # Pre-allocate the global array on storing training data
        X_middle_train_1 = _utils.init_array(X_train_1, self.n_aug_features_)
        X_middle_train_2 = _utils.init_array(X_train_2, self.n_aug_features_)
        # ====================================================================
        # Main loop on the training stage
        # ====================================================================
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_1 = _utils.merge_array(
                X_middle_train_1, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_2 = _utils.merge_array(
                X_middle_train_2, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 2 == 0):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_1, is_training_data=True
                )
            if (layer_idx % 2 == 1):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_2, is_training_data=True
                )
            ...
            # if k == 0:
            #     X_aug2 = np.hstack((X_aug2, X_aug_train_))
            #     print(X_aug2.shape)
            #     print(model2)
            #     my_tensor = torch.from_numpy(X_aug2)
            #     train_features = my_tensor.to(torch.float32)
    ...
    def fit_3Region(self, X_train1, X_train2, X_train3, y, sample_weight=None):
        ...
        X_train_2 = self._bin_data(binner_, X_train2, is_training_data=True)
        X_train_2 = self.buffer_.cache_data(0, X_train_2, is_training_data=True)
        X_train_3 = self._bin_data(binner_, X_train3, is_training_data=True)
        X_train_3 = self.buffer_.cache_data(0, X_train_3, is_training_data=True)
        ...
        X_middle_train_1 = _utils.init_array(X_train_1, self.n_aug_features_)
        X_middle_train_2 = _utils.init_array(X_train_2, self.n_aug_features_)
        X_middle_train_3 = _utils.init_array(X_train_3, self.n_aug_features_)
        ...
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_3 = _utils.merge_array(
                X_middle_train_3, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 3 == 2):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_3, is_training_data=True
                )
                # if k == 1:
                #     X_aug2 = np.hstack((X_aug2, X_aug_train_))
                #     print(X_aug2.shape)
    ...
    def fit_4Region(self, X_train1, X_train2, X_train3, X_train4, y, sample_weight=None):
        ...
        X_train_4 = self._bin_data(binner_, X_train4, is_training_data=True)
        X_train_4 = self.buffer_.cache_data(0, X_train_4, is_training_data=True)
        ...
        X_middle_train_2 = _utils.init_array(X_train_2, self.n_aug_features_)
        X_middle_train_3 = _utils.init_array(X_train_3, self.n_aug_features_)
        X_middle_train_4 = _utils.init_array(X_train_4, self.n_aug_features_)
        # X_middle_train_ = self.buffer_.cache_data(
        #     layer_idx, X_middle_train_1, is_training_data=True
        # )
        # ====================================================================
        ...
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_4 = _utils.merge_array(
                X_middle_train_4, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 4 == 3):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_4, is_training_data=True
                )
            if self.verbose > 0:
                msg = "{} Fitting cascade layer = {:<2}"
                print(msg.format(_utils.ctime(), layer_idx))
    ...
    def fit_5Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, y, sample_weight=None):
        ...
        X_train_5 = self._bin_data(binner_, X_train5, is_training_data=True)
        X_train_5 = self.buffer_.cache_data(0, X_train_5, is_training_data=True)
        ...
        X_middle_train_3 = _utils.init_array(X_train_3, self.n_aug_features_)
        X_middle_train_4 = _utils.init_array(X_train_4, self.n_aug_features_)
        X_middle_train_5 = _utils.init_array(X_train_5, self.n_aug_features_)
        ...
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_5 = _utils.merge_array(
                X_middle_train_5, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 5 == 4):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_5, is_training_data=True
                )
            if self.verbose > 0:
                msg = "{} Fitting cascade layer = {:<2}"
                print(msg.format(_utils.ctime(), layer_idx))
    ...
    def fit_6Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, y, sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
            multi_output=True
        ...
        X_train_5 = self._bin_data(binner_, X_train5, is_training_data=True)
        X_train_5 = self.buffer_.cache_data(0, X_train_5, is_training_data=True)
        X_train_6 = self._bin_data(binner_, X_train6, is_training_data=True)
        X_train_6 = self.buffer_.cache_data(0, X_train_6, is_training_data=True)
        ...
        X_middle_train_4 = _utils.init_array(X_train_4, self.n_aug_features_)
        X_middle_train_5 = _utils.init_array(X_train_5, self.n_aug_features_)
        X_middle_train_6 = _utils.init_array(X_train_6, self.n_aug_features_)
        # ====================================================================
        # Main loop on the training stage
        # ====================================================================
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_6 = _utils.merge_array(
                X_middle_train_6, X_binned_aug_train_, self.n_features_
            )
            ...
            if (layer_idx % 6 == 5):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_6, is_training_data=True
                )
    ...
    def fit_8Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, y, sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
            multi_output=True
        ...
        self._check_input(X_train6, y)
        self._validate_params()
        self._check_input(X_train7, y)
        self._validate_params()
        self._check_input(X_train8, y)
        self._validate_params()
        ...
        X_train_6 = self._bin_data(binner_, X_train6, is_training_data=True)
        X_train_6 = self.buffer_.cache_data(0, X_train_6, is_training_data=True)
        X_train_7 = self._bin_data(binner_, X_train7, is_training_data=True)
        X_train_7 = self.buffer_.cache_data(0, X_train_7, is_training_data=True)
        X_train_8 = self._bin_data(binner_, X_train8, is_training_data=True)
        X_train_8 = self.buffer_.cache_data(0, X_train_8, is_training_data=True)
        ...
        X_middle_train_5 = _utils.init_array(X_train_5, self.n_aug_features_)
        X_middle_train_6 = _utils.init_array(X_train_6, self.n_aug_features_)
        X_middle_train_7 = _utils.init_array(X_train_7, self.n_aug_features_)
        X_middle_train_8 = _utils.init_array(X_train_8, self.n_aug_features_)
        ...
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_7 = _utils.merge_array(
                X_middle_train_7, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_8 = _utils.merge_array(
                X_middle_train_8, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 8 == 6):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_7, is_training_data=True
                )
            if (layer_idx % 8 == 7):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_8, is_training_data=True
                )
    ...
    def fit_9Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, X_train9, y, sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
            multi_output=True
        ...
        X_train_9 = self._bin_data(binner_, X_train9, is_training_data=True)
        X_train_9 = self.buffer_.cache_data(0, X_train_9, is_training_data=True)
        ...
        X_middle_train_7 = _utils.init_array(X_train_7, self.n_aug_features_)
        X_middle_train_8 = _utils.init_array(X_train_8, self.n_aug_features_)
        X_middle_train_9 = _utils.init_array(X_train_9, self.n_aug_features_)
        ...
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_9 = _utils.merge_array(
                X_middle_train_9, X_binned_aug_train_, self.n_features_
            )
            ...
            if (layer_idx % 9 == 8):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_9, is_training_data=True
                )
    ...
    def fit_16Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, X_train9, X_train10, X_train11, X_train12, X_train13, X_train14, X_train15, X_train16, y, sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
            multi_output=True
        ...
        self._check_input(X_train9, y)
        self._validate_params()
        self._check_input(X_train10, y)
        self._validate_params()
        self._check_input(X_train11, y)
        self._validate_params()
        self._check_input(X_train12, y)
        self._validate_params()
        self._check_input(X_train13, y)
        self._validate_params()
        self._check_input(X_train14, y)
        self._validate_params()
        self._check_input(X_train15, y)
        self._validate_params()
        self._check_input(X_train16, y)
        self._validate_params()
        n_counter = 0  # a counter controlling the early stopping
        binner_ = Binner(
            n_bins=self.n_bins,
        ...
        X_train_9 = self._bin_data(binner_, X_train9, is_training_data=True)
        X_train_9 = self.buffer_.cache_data(0, X_train_9, is_training_data=True)
        X_train_10 = self._bin_data(binner_, X_train10, is_training_data=True)
        X_train_10 = self.buffer_.cache_data(0, X_train_10, is_training_data=True)
        X_train_11 = self._bin_data(binner_, X_train11, is_training_data=True)
        X_train_11 = self.buffer_.cache_data(0, X_train_11, is_training_data=True)
        X_train_12 = self._bin_data(binner_, X_train12, is_training_data=True)
        X_train_12 = self.buffer_.cache_data(0, X_train_12, is_training_data=True)

        X_train_13 = self._bin_data(binner_, X_train13, is_training_data=True)
        X_train_13 = self.buffer_.cache_data(0, X_train_13, is_training_data=True)
        X_train_14 = self._bin_data(binner_, X_train14, is_training_data=True)
        X_train_14 = self.buffer_.cache_data(0, X_train_14, is_training_data=True)
        X_train_15 = self._bin_data(binner_, X_train15, is_training_data=True)
        X_train_15 = self.buffer_.cache_data(0, X_train_15, is_training_data=True)
        X_train_16 = self._bin_data(binner_, X_train16, is_training_data=True)
        X_train_16 = self.buffer_.cache_data(0, X_train_16, is_training_data=True)
        # global X_test_1
        # X_test_1 = self._bin_data(binner_, X_test1, is_training_data=False)
        # X_test_1 = self.buffer_.cache_data(0, X_test_1, is_training_data=False)
        # global X_test_2
        ...
        X_middle_train_8 = _utils.init_array(X_train_8, self.n_aug_features_)
        X_middle_train_9 = _utils.init_array(X_train_9, self.n_aug_features_)
        X_middle_train_10 = _utils.init_array(X_train_10, self.n_aug_features_)
        X_middle_train_11 = _utils.init_array(X_train_11, self.n_aug_features_)
        X_middle_train_12 = _utils.init_array(X_train_12, self.n_aug_features_)
        X_middle_train_13 = _utils.init_array(X_train_13, self.n_aug_features_)
        X_middle_train_14 = _utils.init_array(X_train_14, self.n_aug_features_)
        X_middle_train_15 = _utils.init_array(X_train_15, self.n_aug_features_)
        X_middle_train_16 = _utils.init_array(X_train_16, self.n_aug_features_)
        # ====================================================================
        # Main loop on the training stage
        # ====================================================================
        while self.n_layers_ < self.max_layers:
            ...
            X_middle_train_10 = _utils.merge_array(
                X_middle_train_10, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_11 = _utils.merge_array(
                X_middle_train_11, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_12 = _utils.merge_array(
                X_middle_train_12, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_13 = _utils.merge_array(
                X_middle_train_13, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_14 = _utils.merge_array(
                X_middle_train_14, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_15 = _utils.merge_array(
                X_middle_train_15, X_binned_aug_train_, self.n_features_
            )
            X_middle_train_16 = _utils.merge_array(
                X_middle_train_16, X_binned_aug_train_, self.n_features_
            )
            # Build a cascade layer
            layer_idx = self.n_layers_
            layer_ = self._make_layer(
            ...
            if (layer_idx % 16 == 9):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_10, is_training_data=True
                )
            if (layer_idx % 16 == 10):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_11, is_training_data=True
                )
            if (layer_idx % 16 == 11):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_12, is_training_data=True
                )
            if (layer_idx % 16 == 12):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_13, is_training_data=True
                )
            if (layer_idx % 16 == 13):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_14, is_training_data=True
                )
            if (layer_idx % 16 == 14):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_15, is_training_data=True
                )
            if (layer_idx % 16 == 15):
                X_middle_train_ = self.buffer_.cache_data(
                    layer_idx, X_middle_train_16, is_training_data=True
                )
            if self.verbose > 0:
                msg = "{} Fitting cascade layer = {:<2}"
                print(msg.format(_utils.ctime(), layer_idx))
...
class CascadeForestClassifier(BaseCascadeForest, ClassifierMixin):
    ...
    def predict_proba_1Region(self, X_test, y):
        ...
        X_middle_test_ = _utils.init_array(X_test_, self.n_aug_features_)
        for layer_idx in range(self.n_layers_):
            layer = self._get_layer(layer_idx)
            if self.verbose > 0:
                msg = "{} Evaluating cascade layer = {:<2}"
                print(msg.format(_utils.ctime(), layer_idx))
            if layer_idx == 0:
                X_aug_test_ = layer.transform(X_test_)
                X_aug_test = X_aug_test_

                # X_aug_test2 = X_aug_test_
                # print(X_aug_test2.shape)
                # my_tensor = torch.from_numpy(X_aug_test2)
                # test_features = my_tensor.to(torch.float32)
                # my_tensor = torch.from_numpy(y)
                # test_labels = my_tensor.to(torch.float32)
                #
                ...
                # # out = model1[0](data)
                # X_aug_test_ = out
                # X_aug_test_ = X_aug_test_.detach().numpy()

                # my_tensor = torch.from_numpy(X_aug_test)
                # test_features = my_tensor.to(torch.float32)
                # my_tensor = torch.from_numpy(y)
                # test_labels = my_tensor.to(torch.float32)
                #
    ...
    def predict_proba_2Region(self, X_test1, X_test2, y):
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                ...
                # if layer_idx % 2 == 1:
                #     X_aug_test2 = np.hstack((X_aug_test2, X_aug_test_))
                #     print(X_aug_test2.shape)
                #     my_tensor = torch.from_numpy(X_aug_test2)
                #     test_features = my_tensor.to(torch.float32)
                #     my_tensor = torch.from_numpy(y)
    ...
    def predict_proba_4Region(self, X_test1, X_test2, X_test3, X_test4, y):
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                X_aug_test_ = layer.transform(X_test_1)
                X_aug_test = X_aug_test_
                X_middle_test_ = X_middle_test_1
    ...
    def predict_5Region(self, X_test1, X_test2, X_test3, X_test4, X_test5):
        """
        Predict class for X.
        Parameters
        ----------
        ...
        X = check_array(X_test3)
        X = check_array(X_test4)
        X = check_array(X_test5)

        proba, X_aug_test = self.predict_proba_5Region(X_test1, X_test2, X_test3, X_test4, X_test5)
        y = self._decode_class_labels(np.argmax(proba, axis=1))
        return y, proba, X_aug_test

    def fit_6Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, y,
                     sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
        ...
        # Check the input for classification
        y = self._encode_class_labels(y)
        return super().fit_6Region(X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, y, sample_weight)

    def predict_proba_6Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6):
        """
        Predict class probabilities for X.
        Parameters
        ----------
        ...
        X_test_6 = self._bin_data(binner_, X_test6, is_training_data=False)
        X_test_6 = self.buffer_.cache_data(0, X_test_6, is_training_data=False)
        ...
        X_middle_test_4 = _utils.init_array(X_test_4, self.n_aug_features_)
        X_middle_test_5 = _utils.init_array(X_test_5, self.n_aug_features_)
        X_middle_test_6 = _utils.init_array(X_test_6, self.n_aug_features_)
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                ...
                if layer_idx % 6 == 5:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_6, X_aug_test_, self.n_features_
                    )
    ...
    def predict_6Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6):
        """
        Predict class for X.
        Parameters
        ----------
        ...
        X = check_array(X_test4)
        X = check_array(X_test5)
        X = check_array(X_test6)
        proba, X_aug_test = self.predict_proba_6Region(X_test1, X_test2, X_test3, X_test4, X_test5, X_test6)
        y = self._decode_class_labels(np.argmax(proba, axis=1))
        return y, proba, X_aug_test

    def fit_8Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, y,
                     sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
        ...
        # Check the input for classification
        y = self._encode_class_labels(y)
        return super().fit_8Region(X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8,
                                     y, sample_weight)

    def predict_proba_8Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8):
        """
        Predict class probabilities for X.
        Parameters
        ----------
        ...
        X_test5 = check_array(X_test5)
        X_test6 = check_array(X_test6)
        X_test7 = check_array(X_test7)
        X_test8 = check_array(X_test8)
        ...
        self._check_input(X_test5)
        self._check_input(X_test6)
        self._check_input(X_test7)
        self._check_input(X_test8)
        if self.verbose > 0:
            print("{} Start to evalute the model:".format(_utils.ctime()))
        global X_aug_test
        global X_aug_test2
        ...
        X_test_7 = self._bin_data(binner_, X_test7, is_training_data=False)
        X_test_7 = self.buffer_.cache_data(0, X_test_7, is_training_data=False)

        X_test_8 = self._bin_data(binner_, X_test8, is_training_data=False)
        X_test_8 = self.buffer_.cache_data(0, X_test_8, is_training_data=False)
        ...
        X_middle_test_5 = _utils.init_array(X_test_5, self.n_aug_features_)
        X_middle_test_6 = _utils.init_array(X_test_6, self.n_aug_features_)
        X_middle_test_7 = _utils.init_array(X_test_7, self.n_aug_features_)
        X_middle_test_8 = _utils.init_array(X_test_8, self.n_aug_features_)
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                ...
                if layer_idx % 8 == 6:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_7, X_aug_test_, self.n_features_
                    )
                if layer_idx % 8 == 7:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_8, X_aug_test_, self.n_features_
                    )
    ...
    def predict_8Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8):
        """
        Predict class for X.
        Parameters
        ----------
        ...
        X = check_array(X_test5)
        X = check_array(X_test6)
        X = check_array(X_test7)
        X = check_array(X_test8)
        proba, X_aug_test = self.predict_proba_8Region(X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7,
                                                        X_test8)
        y = self._decode_class_labels(np.argmax(proba, axis=1))
        return y, proba, X_aug_test

    def fit_9Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, X_train9, y,
                     sample_weight=None):
        X_train1, y = check_X_y(
            X_train1,
            y,
        ...
        # Check the input for classification
        y = self._encode_class_labels(y)
        return super().fit_9Region(X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8,
                                    X_train9, y, sample_weight)

    def predict_proba_9Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8, X_test9):
        """
        Predict class probabilities for X.
        Parameters
        ----------
        ...
        X_test_9 = self._bin_data(binner_, X_test9, is_training_data=False)
        X_test_9 = self.buffer_.cache_data(0, X_test_9, is_training_data=False)
        ...
        X_middle_test_7 = _utils.init_array(X_test_7, self.n_aug_features_)
        X_middle_test_8 = _utils.init_array(X_test_8, self.n_aug_features_)
        X_middle_test_9 = _utils.init_array(X_test_9, self.n_aug_features_)
        ...
        for layer_idx in range(self.n_layers_):
            ...
            if layer_idx == 0:
                ...
                if layer_idx % 9 == 8:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_9, X_aug_test_, self.n_features_
                    )
    ...
    def predict_9Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8, X_test9):
        """
        Predict class for X.
        Parameters
        ----------
        ...
        X = check_array(X_test7)
        X = check_array(X_test8)
        X = check_array(X_test9)
        proba, X_aug_test = self.predict_proba_9Region(X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7,
                                                        X_test8, X_test9)
        y = self._decode_class_labels(np.argmax(proba, axis=1))
        return y, proba, X_aug_test
    ...
    def fit_16Region(self, X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, X_train9, X_train10, X_train11, X_train12, X_train13, X_train14, X_train15, X_train16, y, sample_weight=None):
        ...
        # Check the input for classification
        y = self._encode_class_labels(y)
        return super().fit_16Region(X_train1, X_train2, X_train3, X_train4, X_train5, X_train6, X_train7, X_train8, X_train9, X_train10, X_train11, X_train12, X_train13, X_train14, X_train15, X_train16, y, sample_weight)

    def predict_proba_16Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8, X_test9, X_test10, X_test11, X_test12, X_test13, X_test14, X_test15, X_test16):
        """
        Predict class probabilities for X.
        Parameters
        ----------
        ...
        X_test8 = check_array(X_test8)
        X_test9 = check_array(X_test9)
        X_test10 = check_array(X_test10)
        X_test11 = check_array(X_test11)
        X_test12 = check_array(X_test12)
        X_test13 = check_array(X_test13)
        X_test14 = check_array(X_test14)
        X_test15 = check_array(X_test15)
        X_test16 = check_array(X_test16)
        ...
        self._check_input(X_test8)
        self._check_input(X_test9)
        self._check_input(X_test10)
        self._check_input(X_test11)
        self._check_input(X_test12)
        self._check_input(X_test13)
        self._check_input(X_test14)
        self._check_input(X_test15)
        self._check_input(X_test16)
        if self.verbose > 0:
            print("{} Start to evalute the model:".format(_utils.ctime()))
        global X_aug_test
        global X_aug_test2
        ...
        X_test_10 = self._bin_data(binner_, X_test10, is_training_data=False)
        X_test_10 = self.buffer_.cache_data(0, X_test_10, is_training_data=False)

        X_test_11 = self._bin_data(binner_, X_test11, is_training_data=False)
        X_test_11 = self.buffer_.cache_data(0, X_test_11, is_training_data=False)

        X_test_12 = self._bin_data(binner_, X_test12, is_training_data=False)
        X_test_12 = self.buffer_.cache_data(0, X_test_12, is_training_data=False)

        X_test_13 = self._bin_data(binner_, X_test13, is_training_data=False)
        X_test_13 = self.buffer_.cache_data(0, X_test_13, is_training_data=False)

        X_test_14 = self._bin_data(binner_, X_test14, is_training_data=False)
        X_test_14 = self.buffer_.cache_data(0, X_test_14, is_training_data=False)

        X_test_15 = self._bin_data(binner_, X_test15, is_training_data=False)
        X_test_15 = self.buffer_.cache_data(0, X_test_15, is_training_data=False)

        X_test_16 = self._bin_data(binner_, X_test16, is_training_data=False)
        X_test_16 = self.buffer_.cache_data(0, X_test_16, is_training_data=False)
        ...
        X_middle_test_8 = _utils.init_array(X_test_8, self.n_aug_features_)
        X_middle_test_9 = _utils.init_array(X_test_9, self.n_aug_features_)
        X_middle_test_10 = _utils.init_array(X_test_10, self.n_aug_features_)
        X_middle_test_11 = _utils.init_array(X_test_11, self.n_aug_features_)
        X_middle_test_12 = _utils.init_array(X_test_12, self.n_aug_features_)
        X_middle_test_13 = _utils.init_array(X_test_13, self.n_aug_features_)
        X_middle_test_14 = _utils.init_array(X_test_14, self.n_aug_features_)
        X_middle_test_15 = _utils.init_array(X_test_15, self.n_aug_features_)
        X_middle_test_16 = _utils.init_array(X_test_16, self.n_aug_features_)
        for layer_idx in range(self.n_layers_):
            layer = self._get_layer(layer_idx)
            if self.verbose > 0:
                msg = "{} Evaluating cascade layer = {:<2}"
            ...
            if layer_idx == 0:
                ...
                if layer_idx % 16 == 9:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_10, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 10:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_11, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 11:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_12, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 12:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_13, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 13:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_14, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 14:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_15, X_aug_test_, self.n_features_
                    )
                if layer_idx % 16 == 15:
                    X_middle_test_ = _utils.merge_array(
                        X_middle_test_16, X_aug_test_, self.n_features_
                    )
    ...
    def predict_16Region(self, X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8, X_test9, X_test10, X_test11, X_test12, X_test13, X_test14, X_test15, X_test16):
        """
        Predict class for X.
        Parameters
        ----------
        ...
        X = check_array(X_test8)
        X = check_array(X_test9)
        X = check_array(X_test10)
        X = check_array(X_test11)
        X = check_array(X_test12)
        X = check_array(X_test13)
        X = check_array(X_test14)
        X = check_array(X_test15)
        X = check_array(X_test16)
        proba, X_aug_test = self.predict_proba_16Region(X_test1, X_test2, X_test3, X_test4, X_test5, X_test6, X_test7, X_test8, X_test9, X_test10, X_test11, X_test12, X_test13, X_test14, X_test15, X_test16)
        y = self._decode_class_labels(np.argmax(proba, axis=1))
        return y, proba, X_aug_test
